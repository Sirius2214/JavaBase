$(function () {
    ...
    $('#save').click(function () {
        // var form = getForm();
        // form.model = model;
        var infoArray = $("#info").serializeArray();
        infoArray.push({
            name:'model',
            value:model,
        })
        var form=getFormJson(infoArray)
...
function refreshParentTable() {
    ...
    $('[id="tb_body"]',window.opener.document).empty();
    $('[id="tb_body"]',window.opener.document).append(data.responseText)
    // //清空该标签下的所有数据
