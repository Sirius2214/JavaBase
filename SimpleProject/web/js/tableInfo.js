...
$(function () {
    ...
    //获取表格数据
    pushTable(getAjax('/tableInfo', 'Post', null))
...
function clickDepart(len, index) {
    ...
    console.log(depart)
    //获取当前部门数据
    var data = getAjax('/GetUserInfoByDepart',"POST",{depart:encodeStr(depart)})
    //属性表格
    pushTable(data)
...
function search(len) {
    ...
    if (!userID) {
        ...
        let data = getAjax('/getByName', 'POST', to)
